...
# Modified from: http://esd.io/blog/flask-apps-heroku-real-ip-spoofing.html
class SaferProxyFix(object):
    ...
    def __init__(self,
                 app, num_proxy_servers=1, detect_misconfiguration=False):
        self.app = app
    ...
    def get_remote_addr(self, forwarded_for):
        ...
        if self.detect_misconfiguration and not forwarded_for:
            raise Exception(
                "SaferProxyFix did not detect a proxy server. Do not use this"
                " fixer if you are not behind a proxy.")
        if self.detect_misconfiguration \
                and len(forwarded_for) < self.num_proxy_servers:
            raise Exception(
                "SaferProxyFix did not detect enough proxy servers. Check your"
                " num_proxy_servers setting.")
